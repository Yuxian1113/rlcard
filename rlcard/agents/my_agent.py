...
# predict q values of current state and determine the best action 
	...
	def train(self):
		...
		q_value = self.policy_net(next_state_batch)
		Vt = q_value.gather(1, torch.tensor(batch.action, device=self.device).unsqueeze(1)).view(-1) 
		preferred_action = q_value.max(1)[1]
		Vt_next = self.target_net(next_state_batch).gather(1, preferred_action.unsqueeze(1)).squeeze()
